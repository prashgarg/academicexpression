...
# Revised plot with shaded region along a line
p_combined <- ggplot(results_df_combined %>% filter(Group == "Main"), aes(x = Date, y = value)) +
  geom_smooth(
    method = "loess", formula = 'y ~ x', se = TRUE, color = "black", size = 0.5,
    fill = "grey80", alpha = 0.3  # Shaded region for variability
  ) +
  geom_hline(data = hline_data, aes(yintercept = yintercept), linetype = "dotted", color = "grey99", size = 0.5,alpha=0) +  # Add horizontal lines
  labs(y = "Behavioural Expression", x = "Year by Month") +
  facet_wrap(~Label, scales = "free_y", nrow = 3, ncol = 3) +  # 3x3 layout
  theme_classic(base_size = 7) +  # Journal-compliant font size
  theme(
    text = element_text(family = "Helvetica"),  # Use Helvetica font
    strip.text = element_text(face = "bold", size = 7),  # Adjust facet label size
    ...
    panel.background = element_blank()  # Ensure clean panel backgrounds
  )
...
# Revised plot with two series and shaded regions
p_combined2 <- ggplot(results_df_combined2, aes(x = Date, y = value, color = Type, fill = Type)) +
  ...
  geom_hline(data = hline_data2, aes(yintercept = yintercept), linetype = "dotted", color = "grey99",alpha=0, size = 0.1) +  # Add horizontal lines
