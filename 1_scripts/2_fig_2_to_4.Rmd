...
Sample of U.S. based academics
```{r}
repl_df_us<-repl_df %>%  filter(affiliation_country_code=="US") 
```
...
```{r}
# Map the function over each variable group and combine results
stats_pol_main <- map_df(clim_var, ~calculate_stats(repl_df_us, .x))

stats_pol_is_female <- map_df(clim_var, ~calculate_stats(repl_df_us %>% filter(is_female==1), .x))
stats_pol_is_male <- map_df(clim_var, ~calculate_stats(repl_df_us %>% filter(is_male==1), .x))

stats_pol_is_stem <- map_df(clim_var, ~calculate_stats(repl_df_us %>% filter(is_stem==1), .x))
stats_pol_is_soc_sci <- map_df(clim_var, ~calculate_stats(repl_df_us %>% filter(is_soc_sci==1), .x))
stats_pol_is_humanities <- map_df(clim_var, ~calculate_stats(repl_df_us %>% filter(is_humanities==1), .x))

stats_pol_type_H_reach_H_exp_climate <- map_df(clim_var, ~calculate_stats(repl_df_us %>% filter(type_H_reach_H_exp_climate==1), .x))
stats_pol_type_H_reach_L_exp_climate <- map_df(clim_var, ~calculate_stats(repl_df_us %>% filter(type_H_reach_L_exp_climate==1), .x))
stats_pol_type_L_reach_H_exp_climate <- map_df(clim_var, ~calculate_stats(repl_df_us %>% filter(type_L_reach_H_exp_climate==1), .x))
stats_pol_type_L_reach_L_exp_climate <- map_df(clim_var, ~calculate_stats(repl_df_us %>% filter(type_L_reach_L_exp_climate==1), .x))

stats_pol_ranking_1_100 <- map_df(clim_var, ~calculate_stats(repl_df_us %>% filter(ranking_cat == "1-100"), .x))
stats_pol_ranking_1_100$Type <- "1-100"

stats_pol_ranking_101_500 <- map_df(clim_var, ~calculate_stats(repl_df_us %>% filter(ranking_cat == "101-500"), .x))
stats_pol_ranking_101_500$Type <- "101-500"

stats_pol_ranking_501_1500 <- map_df(clim_var, ~calculate_stats(repl_df_us %>% filter(ranking_cat == "501-1500"), .x))
stats_pol_ranking_501_1500$Type <- "501-1500"

stats_pol_ranking_1501 <- map_df(clim_var, ~calculate_stats(repl_df_us %>% filter(ranking_cat == "1501+"), .x))
stats_pol_ranking_1501$Type <- "1501+"
...
stats_pol_comb <- bind_rows(stats_pol_main, stats_pol_field, stats_pol_reach_by_exp_climate, stats_pol_gender, stats_pol_rank, stats_pol_us_users)
...
```{r}
# Map the function over each variable group and combine results
stats_pol_main <- map_df(econ_var, ~calculate_stats(repl_df_us, .x))

stats_pol_is_female <- map_df(econ_var, ~calculate_stats(repl_df_us %>% filter(is_female==1), .x))
stats_pol_is_male <- map_df(econ_var, ~calculate_stats(repl_df_us %>% filter(is_male==1), .x))

stats_pol_is_stem <- map_df(econ_var, ~calculate_stats(repl_df_us %>% filter(is_stem==1), .x))
stats_pol_is_soc_sci <- map_df(econ_var, ~calculate_stats(repl_df_us %>% filter(is_soc_sci==1), .x))
stats_pol_is_humanities <- map_df(econ_var, ~calculate_stats(repl_df_us %>% filter(is_humanities==1), .x))

stats_pol_type_H_reach_H_exp_econ <- map_df(econ_var, ~calculate_stats(repl_df_us %>% filter(type_H_reach_H_exp_econ==1), .x))
stats_pol_type_H_reach_L_exp_econ <- map_df(econ_var, ~calculate_stats(repl_df_us %>% filter(type_H_reach_L_exp_econ==1), .x))
stats_pol_type_L_reach_H_exp_econ <- map_df(econ_var, ~calculate_stats(repl_df_us %>% filter(type_L_reach_H_exp_econ==1), .x))
stats_pol_type_L_reach_L_exp_econ <- map_df(econ_var, ~calculate_stats(repl_df_us %>% filter(type_L_reach_L_exp_econ==1), .x))

stats_pol_type_H_reach_H_exp_cult <- map_df(econ_var, ~calculate_stats(repl_df_us %>% filter(type_H_reach_H_exp_cult==1), .x))
stats_pol_type_H_reach_L_exp_cult <- map_df(econ_var, ~calculate_stats(repl_df_us %>% filter(type_H_reach_L_exp_cult==1), .x))
stats_pol_type_L_reach_H_exp_cult <- map_df(econ_var, ~calculate_stats(repl_df_us %>% filter(type_L_reach_H_exp_cult==1), .x))
stats_pol_type_L_reach_L_exp_cult <- map_df(econ_var, ~calculate_stats(repl_df_us %>% filter(type_L_reach_L_exp_cult==1), .x))

stats_pol_ranking_1_100 <- map_df(econ_var, ~calculate_stats(repl_df_us %>% filter(ranking_cat == "1-100"), .x))
stats_pol_ranking_1_100$Type <- "1-100"

stats_pol_ranking_101_500 <- map_df(econ_var, ~calculate_stats(repl_df_us %>% filter(ranking_cat == "101-500"), .x))
stats_pol_ranking_101_500$Type <- "101-500"

stats_pol_ranking_501_1500 <- map_df(econ_var, ~calculate_stats(repl_df_us %>% filter(ranking_cat == "501-1500"), .x))
stats_pol_ranking_501_1500$Type <- "501-1500"

stats_pol_ranking_1501 <- map_df(econ_var, ~calculate_stats(repl_df_us %>% filter(ranking_cat == "1501+"), .x))
stats_pol_ranking_1501$Type <- "1501+"
...
stats_pol_comb <- bind_rows(stats_pol_main, stats_pol_field, 
                            stats_pol_reach_by_exp_econ %>%mutate(exp="econ"), 
                            stats_pol_reach_by_exp_cult%>%mutate(exp="cult"), 
                            stats_pol_gender, stats_pol_rank, stats_pol_us_users)
...
```{r}
# Map the function over each variable group and combine results
stats_pol_main <- map_df(beh_var, ~calculate_stats(repl_df_us, .x))

stats_pol_is_female <- map_df(beh_var, ~calculate_stats(repl_df_us %>% filter(is_female==1), .x))
stats_pol_is_male <- map_df(beh_var, ~calculate_stats(repl_df_us %>% filter(is_male==1), .x))

stats_pol_is_stem <- map_df(beh_var, ~calculate_stats(repl_df_us %>% filter(is_stem==1), .x))
stats_pol_is_soc_sci <- map_df(beh_var, ~calculate_stats(repl_df_us %>% filter(is_soc_sci==1), .x))
stats_pol_is_humanities <- map_df(beh_var, ~calculate_stats(repl_df_us %>% filter(is_humanities==1), .x))

stats_pol_type_H_reach_H_cred <- map_df(beh_var, ~calculate_stats(repl_df_us %>% filter(type_H_reach_H_cred==1), .x))
stats_pol_type_H_reach_L_cred <- map_df(beh_var, ~calculate_stats(repl_df_us %>% filter(type_H_reach_L_cred==1), .x))
stats_pol_type_L_reach_H_cred <- map_df(beh_var, ~calculate_stats(repl_df_us %>% filter(type_L_reach_H_cred==1), .x))
stats_pol_type_L_reach_L_cred <- map_df(beh_var, ~calculate_stats(repl_df_us %>% filter(type_L_reach_L_cred==1), .x))

stats_pol_ranking_1_100 <- map_df(beh_var, ~calculate_stats(repl_df_us %>% filter(ranking_cat == "1-100"), .x))
stats_pol_ranking_1_100$Type <- "1-100"

stats_pol_ranking_101_500 <- map_df(beh_var, ~calculate_stats(repl_df_us %>% filter(ranking_cat == "101-500"), .x))
stats_pol_ranking_101_500$Type <- "101-500"

stats_pol_ranking_501_1500 <- map_df(beh_var, ~calculate_stats(repl_df_us %>% filter(ranking_cat == "501-1500"), .x))
stats_pol_ranking_501_1500$Type <- "501-1500"

stats_pol_ranking_1501 <- map_df(beh_var, ~calculate_stats(repl_df_us %>% filter(ranking_cat == "1501+"), .x))
stats_pol_ranking_1501$Type <- "1501+"
...
stats_pol_comb <- bind_rows(stats_pol_main, stats_pol_field, stats_pol_reach_by_cred, stats_pol_gender, stats_pol_rank, stats_pol_us_users)
...
techno opt
```{r}
# High-reach non-experts (low expertise) vs high-reach experts on TO stance
t.test(pro_techno_prop ~ type_H_reach_H_exp_climate, data = repl_df %>% filter(type_H_reach_H_exp_climate == 1 | type_H_reach_L_exp_climate == 1))
cohen.d(pro_techno_prop ~ type_H_reach_H_exp_climate, data = repl_df %>% filter(type_H_reach_H_exp_climate == 1 | type_H_reach_L_exp_climate == 1))

# Low-reach subject matter experts vs high-reach non-experts on TO stance
t.test(pro_techno_prop ~ type_L_reach_H_exp_climate, data = repl_df %>% filter(type_L_reach_H_exp_climate == 1 | type_H_reach_L_exp_climate == 1))
cohen.d(pro_techno_prop ~ type_L_reach_H_exp_climate, data = repl_df %>% filter(type_L_reach_H_exp_climate == 1 | type_H_reach_L_exp_climate == 1))
...
beh adj
```{r}
# High-reach non-experts (low expertise) vs high-reach experts on BA stance
t.test(pro_beh_prop ~ type_H_reach_H_exp_climate, data = repl_df %>% filter(type_H_reach_H_exp_climate == 1 | type_H_reach_L_exp_climate == 1))
cohen.d(pro_beh_prop ~ type_H_reach_H_exp_climate, data = repl_df %>% filter(type_H_reach_H_exp_climate == 1 | type_H_reach_L_exp_climate == 1))

# Low-reach subject matter experts vs high-reach non-experts on BA stance
t.test(pro_beh_prop ~ type_L_reach_H_exp_climate, data = repl_df %>% filter(type_L_reach_H_exp_climate == 1 | type_H_reach_L_exp_climate == 1))
cohen.d(pro_beh_prop ~ type_L_reach_H_exp_climate, data = repl_df %>% filter(type_L_reach_H_exp_climate == 1 | type_H_reach_L_exp_climate == 1))



```

Normality and other tests for assumptions
```{r}
...
compare_groups <- function(data, outcome, group_var, group_val, ref_val) {
  ...
  # Levene’s test
  levene_out <- check_variance_homogeneity(c(group1, group2), 
               c(rep("Group1", length(group1)), rep("Group2", length(group2))))
...
#-------------------------------------------
# Climate Action Comparisons
#-------------------------------------------
# Comparison 1: High-reach experts vs. high-reach non-experts 
res_ca_hr <- compare_groups(
  data = repl_df %>% filter(type_H_reach_H_exp_climate == 1 | type_H_reach_L_exp_climate == 1),
  outcome = "stance_climate_action",
  group_var = "type_H_reach_H_exp_climate",
  group_val = 1,
  ref_val = 0
)
cat("=== Climate Action: High-reach experts vs. high-reach non-experts ===\n")
cat("Group 1 (High-reach experts): N =", res_ca_hr$group1_stats$n, 
    ", Mean =", round(res_ca_hr$group1_stats$mean, 4),
    ", SD =", round(res_ca_hr$group1_stats$sd, 4),
    ", 95% CI = [", round(res_ca_hr$group1_stats$ci_lower, 4), ",", round(res_ca_hr$group1_stats$ci_upper, 4), "]\n")
cat("Skewness =", round(res_ca_hr$normality$group1$skewness, 3), 
    ", Kurtosis =", round(res_ca_hr$normality$group1$kurtosis, 3), "\n")
cat("Group 2 (High-reach non-experts): N =", res_ca_hr$group2_stats$n, 
    ", Mean =", round(res_ca_hr$group2_stats$mean, 4),
    ", SD =", round(res_ca_hr$group2_stats$sd, 4),
    ", 95% CI = [", round(res_ca_hr$group2_stats$ci_lower, 4), ",", round(res_ca_hr$group2_stats$ci_upper, 4), "]\n")
cat("Skewness =", round(res_ca_hr$normality$group2$skewness, 3), 
    ", Kurtosis =", round(res_ca_hr$normality$group2$kurtosis, 3), "\n")
cat("Levene's Test: F(", res_ca_hr$levene$df1, ",", res_ca_hr$levene$df2, ") = ",
    round(res_ca_hr$levene$f_value, 2), ", p =", formatC(res_ca_hr$levene$p_value, format = "f", digits = 4), "\n")
cat("Welch's t-test: t(", round(res_ca_hr$t_test$parameter, 2), ") = ", 
    round(res_ca_hr$t_test$statistic, 3), ", p =", formatC(res_ca_hr$t_test$p.value, format = "f", digits = 4), 
    ", Mean Diff =", round(diff(res_ca_hr$t_test$estimate), 4), "\n")
cat("Cohen's d:", round(res_ca_hr$cohen_d, 3), "\n\n")


# Comparison 2: Low-reach experts vs. high-reach non-experts 
res_ca_lr <- compare_groups(
  data = repl_df %>% filter(type_L_reach_H_exp_climate == 1 | type_H_reach_L_exp_climate == 1),
  outcome = "stance_climate_action",
  group_var = "type_L_reach_H_exp_climate",
  group_val = 1,
  ref_val = 0
)
cat("=== Climate Action: Low-reach experts vs. high-reach non-experts ===\n")
cat("Group 1 (Low-reach experts): N =", res_ca_lr$group1_stats$n, 
    ", Mean =", round(res_ca_lr$group1_stats$mean, 4),
    ", SD =", round(res_ca_lr$group1_stats$sd, 4),
    ", 95% CI = [", round(res_ca_lr$group1_stats$ci_lower, 4), ",", round(res_ca_lr$group1_stats$ci_upper, 4), "]\n")
cat("Skewness =", round(res_ca_lr$normality$group1$skewness, 3), 
    ", Kurtosis =", round(res_ca_lr$normality$group1$kurtosis, 3), "\n")
cat("Group 2 (High-reach non-experts): N =", res_ca_lr$group2_stats$n, 
    ", Mean =", round(res_ca_lr$group2_stats$mean, 4),
    ", SD =", round(res_ca_lr$group2_stats$sd, 4),
    ", 95% CI = [", round(res_ca_lr$group2_stats$ci_lower, 4), ",", round(res_ca_lr$group2_stats$ci_upper, 4), "]\n")
cat("Skewness =", round(res_ca_lr$normality$group2$skewness, 3), 
    ", Kurtosis =", round(res_ca_lr$normality$group2$kurtosis, 3), "\n")
cat("Levene's Test: F(", res_ca_lr$levene$df1, ",", res_ca_lr$levene$df2, ") = ",
    round(res_ca_lr$levene$f_value, 2), ", p =", formatC(res_ca_lr$levene$p_value, format = "f", digits = 4), "\n")
cat("Welch's t-test: t(", round(res_ca_lr$t_test$parameter, 2), ") = ", 
    round(res_ca_lr$t_test$statistic, 3), ", p =", formatC(res_ca_lr$t_test$p.value, format = "f", digits = 4), 
    ", Mean Diff =", round(diff(res_ca_lr$t_test$estimate), 4), "\n")
cat("Cohen's d:", round(res_ca_lr$cohen_d, 3), "\n\n")


#-------------------------------------------
# Techno-Optimism Comparisons
#-------------------------------------------
# Comparison 1: High-reach experts vs. high-reach non-experts 
res_to_hr <- compare_groups(
  data = repl_df %>% filter(type_H_reach_H_exp_climate == 1 | type_H_reach_L_exp_climate == 1),
  outcome = "pro_techno_prop",
  group_var = "type_H_reach_H_exp_climate",
  group_val = 1,
  ref_val = 0
)
cat("=== Techno-Optimism: High-reach experts vs. high-reach non-experts ===\n")
cat("Group 1 (High-reach experts): N =", res_to_hr$group1_stats$n, 
    ", Mean =", round(res_to_hr$group1_stats$mean, 4),
    ", SD =", round(res_to_hr$group1_stats$sd, 4),
    ", 95% CI = [", round(res_to_hr$group1_stats$ci_lower, 4), ",", round(res_to_hr$group1_stats$ci_upper, 4), "]\n")
cat("Skewness =", round(res_to_hr$normality$group1$skewness, 3), 
    ", Kurtosis =", round(res_to_hr$normality$group1$kurtosis, 3), "\n")
cat("Group 2 (High-reach non-experts): N =", res_to_hr$group2_stats$n, 
    ", Mean =", round(res_to_hr$group2_stats$mean, 4),
    ", SD =", round(res_to_hr$group2_stats$sd, 4),
    ", 95% CI = [", round(res_to_hr$group2_stats$ci_lower, 4), ",", round(res_to_hr$group2_stats$ci_upper, 4), "]\n")
cat("Skewness =", round(res_to_hr$normality$group2$skewness, 3), 
    ", Kurtosis =", round(res_to_hr$normality$group2$kurtosis, 3), "\n")
cat("Levene's Test: F(", res_to_hr$levene$df1, ",", res_to_hr$levene$df2, ") = ",
    round(res_to_hr$levene$f_value, 2), ", p =", formatC(res_to_hr$levene$p_value, format = "f", digits = 4), "\n")
cat("Welch's t-test: t(", round(res_to_hr$t_test$parameter, 2), ") = ", 
    round(res_to_hr$t_test$statistic, 3), ", p =", formatC(res_to_hr$t_test$p.value, format = "f", digits = 4), 
    ", Mean Diff =", round(diff(res_to_hr$t_test$estimate), 4), "\n")
cat("Cohen's d:", round(res_to_hr$cohen_d, 3), "\n\n")

# Comparison 2: Low-reach experts vs. high-reach non-experts 
res_to_lr <- compare_groups(
  data = repl_df %>% filter(type_L_reach_H_exp_climate == 1 | type_H_reach_L_exp_climate == 1),
  outcome = "pro_techno_prop",
  group_var = "type_L_reach_H_exp_climate",
  group_val = 1,
...
cat("=== Techno-Optimism: Low-reach experts vs. high-reach non-experts ===\n")
cat("Group 1 (Low-reach experts): N =", res_to_lr$group1_stats$n, 
    ", Mean =", round(res_to_lr$group1_stats$mean, 4),
    ", SD =", round(res_to_lr$group1_stats$sd, 4),
    ", 95% CI = [", round(res_to_lr$group1_stats$ci_lower, 4), ",", round(res_to_lr$group1_stats$ci_upper, 4), "]\n")
cat("Skewness =", round(res_to_lr$normality$group1$skewness, 3), 
    ", Kurtosis =", round(res_to_lr$normality$group1$kurtosis, 3), "\n")
cat("Group 2 (High-reach non-experts): N =", res_to_lr$group2_stats$n, 
    ", Mean =", round(res_to_lr$group2_stats$mean, 4),
    ", SD =", round(res_to_lr$group2_stats$sd, 4),
    ", 95% CI = [", round(res_to_lr$group2_stats$ci_lower, 4), ",", round(res_to_lr$group2_stats$ci_upper, 4), "]\n")
cat("Skewness =", round(res_to_lr$normality$group2$skewness, 3), 
    ", Kurtosis =", round(res_to_lr$normality$group2$kurtosis, 3), "\n")
cat("Levene's Test: F(", res_to_lr$levene$df1, ",", res_to_lr$levene$df2, ") = ",
    round(res_to_lr$levene$f_value, 2), ", p =", formatC(res_to_lr$levene$p_value, format = "f", digits = 4), "\n")
cat("Welch's t-test: t(", round(res_to_lr$t_test$parameter, 2), ") = ", 
    round(res_to_lr$t_test$statistic, 3), ", p =", formatC(res_to_lr$t_test$p.value, format = "f", digits = 4), 
    ", Mean Diff =", round(diff(res_to_lr$t_test$estimate), 4), "\n")
cat("Cohen's d:", round(res_to_lr$cohen_d, 3), "\n\n")

#-------------------------------------------
# Behavioral Adjustment Comparisons
#-------------------------------------------
# Comparison 1: High-reach experts vs. high-reach non-experts 
res_ba_hr <- compare_groups(
  data = repl_df %>% filter(type_H_reach_H_exp_climate == 1 | type_H_reach_L_exp_climate == 1),
  outcome = "pro_beh_prop",
  group_var = "type_H_reach_H_exp_climate",
  group_val = 1,
  ref_val = 0
)
cat("=== Behavioral Adjustment: High-reach experts vs. high-reach non-experts ===\n")
cat("Group 1 (High-reach experts): N =", res_ba_hr$group1_stats$n, 
    ", Mean =", round(res_ba_hr$group1_stats$mean, 4),
    ", SD =", round(res_ba_hr$group1_stats$sd, 4),
    ", 95% CI = [", round(res_ba_hr$group1_stats$ci_lower, 4), ",", round(res_ba_hr$group1_stats$ci_upper, 4), "]\n")
cat("Skewness =", round(res_ba_hr$normality$group1$skewness, 3), 
    ", Kurtosis =", round(res_ba_hr$normality$group1$kurtosis, 3), "\n")
cat("Group 2 (High-reach non-experts): N =", res_ba_hr$group2_stats$n, 
    ", Mean =", round(res_ba_hr$group2_stats$mean, 4),
    ", SD =", round(res_ba_hr$group2_stats$sd, 4),
    ", 95% CI = [", round(res_ba_hr$group2_stats$ci_lower, 4), ",", round(res_ba_hr$group2_stats$ci_upper, 4), "]\n")
cat("Skewness =", round(res_ba_hr$normality$group2$skewness, 3), 
    ", Kurtosis =", round(res_ba_hr$normality$group2$kurtosis, 3), "\n")
cat("Levene's Test: F(", res_ba_hr$levene$df1, ",", res_ba_hr$levene$df2, ") = ",
    round(res_ba_hr$levene$f_value, 2), ", p =", formatC(res_ba_hr$levene$p_value, format = "f", digits = 4), "\n")
cat("Welch's t-test: t(", round(res_ba_hr$t_test$parameter, 2), ") = ", 
    round(res_ba_hr$t_test$statistic, 3), ", p =", formatC(res_ba_hr$t_test$p.value, format = "f", digits = 4), 
    ", Mean Diff =", round(diff(res_ba_hr$t_test$estimate), 4), "\n")
cat("Cohen's d:", round(res_ba_hr$cohen_d, 3), "\n\n")

# Comparison 2: Low-reach experts vs. high-reach non-experts 
res_ba_lr <- compare_groups(
  data = repl_df %>% filter(type_L_reach_H_exp_climate == 1 | type_H_reach_L_exp_climate == 1),
  outcome = "pro_beh_prop",
  group_var = "type_L_reach_H_exp_climate",
  group_val = 1,
...
cat("=== Behavioral Adjustment: Low-reach experts vs. high-reach non-experts ===\n")
cat("Group 1 (Low-reach experts): N =", res_ba_lr$group1_stats$n, 
    ", Mean =", round(res_ba_lr$group1_stats$mean, 4),
    ", SD =", round(res_ba_lr$group1_stats$sd, 4),
    ", 95% CI = [", round(res_ba_lr$group1_stats$ci_lower, 4), ",", round(res_ba_lr$group1_stats$ci_upper, 4), "]\n")
cat("Skewness =", round(res_ba_lr$normality$group1$skewness, 3), 
    ", Kurtosis =", round(res_ba_lr$normality$group1$kurtosis, 3), "\n")
cat("Group 2 (High-reach non-experts): N =", res_ba_lr$group2_stats$n, 
    ", Mean =", round(res_ba_lr$group2_stats$mean, 4),
    ", SD =", round(res_ba_lr$group2_stats$sd, 4),
    ", 95% CI = [", round(res_ba_lr$group2_stats$ci_lower, 4), ",", round(res_ba_lr$group2_stats$ci_upper, 4), "]\n")
cat("Skewness =", round(res_ba_lr$normality$group2$skewness, 3), 
    ", Kurtosis =", round(res_ba_lr$normality$group2$kurtosis, 3), "\n")
cat("Levene's Test: F(", res_ba_lr$levene$df1, ",", res_ba_lr$levene$df2, ") = ",
    round(res_ba_lr$levene$f_value, 2), ", p =", formatC(res_ba_lr$levene$p_value, format = "f", digits = 4), "\n")
cat("Welch's t-test: t(", round(res_ba_lr$t_test$parameter, 2), ") = ", 
    round(res_ba_lr$t_test$statistic, 3), ", p =", formatC(res_ba_lr$t_test$p.value, format = "f", digits = 4), 
    ", Mean Diff =", round(diff(res_ba_lr$t_test$estimate), 4), "\n")
cat("Cohen's d:", round(res_ba_lr$cohen_d, 3), "\n\n")
...
```{r}
# Create indicator variables for ranking categories (for any analyses that may need them)
repl_df <- repl_df %>%
...
detailed_comparison <- function(data, outcome, groupvar, group1_label, group2_label) {
  # Subset data based on binary indicator in groupvar
  ...
  # Levene’s test for variance homogeneity (explicitly convert group to factor)
  combined_data <- data.frame(
    value = c(data_group1[[outcome]], data_group2[[outcome]]),
    group = factor(c(rep(group1_label, n1), rep(group2_label, n2)))
  )
...
# --- US vs. Non-US Comparisons ---

# Climate Action: U.S.-based vs. Non-U.S.-based
detailed_comparison(repl_df, "stance_climate_action", "is_US", "U.S.-based", "Non-U.S.-based")

# Behavioral Adjustments: U.S.-based vs. Non-U.S.-based
detailed_comparison(repl_df, "pro_beh_prop", "is_US", "U.S.-based", "Non-U.S.-based")

# Techno Opt: U.S.-based vs. Non-U.S.-based
detailed_comparison(repl_df, "pro_techno_prop", "is_US", "U.S.-based", "Non-U.S.-based")

# --- University Rankings Comparisons ---
# For each comparison, we subset to only include the two groups of interest 
# and create a new binary indicator (is_top100_subset) for Top 100 vs. the other group.


##### climate action
# University Rankings: 1-100 vs. 101-500
ranking_subset <- repl_df %>% 
  filter(ranking_cat %in% c("1-100", "101-500")) %>%
  mutate(is_top100_subset = ifelse(ranking_cat == "1-100", 1, 0))
detailed_comparison(ranking_subset, "stance_climate_action", "is_top100_subset", "Top 100 Universities", "101-500 Universities")

# University Rankings: 1-100 vs. 501-1500
ranking_subset <- repl_df %>% 
  filter(ranking_cat %in% c("1-100", "501-1500")) %>%
  mutate(is_top100_subset = ifelse(ranking_cat == "1-100", 1, 0))
detailed_comparison(ranking_subset, "stance_climate_action", "is_top100_subset", "Top 100 Universities", "501-1500 Universities")

# University Rankings: 1-100 vs. 1501+
ranking_subset <- repl_df %>% 
  filter(ranking_cat %in% c("1-100", "1501+")) %>%
  mutate(is_top100_subset = ifelse(ranking_cat == "1-100", 1, 0))
detailed_comparison(ranking_subset, "stance_climate_action", "is_top100_subset", "Top 100 Universities", "1501+ Universities")
...
ranking_subset <- repl_df %>% 
  filter(ranking_cat %in% c("1-100", "101-500")) %>%
  mutate(is_top100_subset = ifelse(ranking_cat == "1-100", 1, 0))
detailed_comparison(ranking_subset, "pro_techno_prop", "is_top100_subset", "Top 100 Universities", "101-500 Universities")

# University Rankings: 1-100 vs. 501-1500
ranking_subset <- repl_df %>% 
  filter(ranking_cat %in% c("1-100", "501-1500")) %>%
  mutate(is_top100_subset = ifelse(ranking_cat == "1-100", 1, 0))
detailed_comparison(ranking_subset, "pro_techno_prop", "is_top100_subset", "Top 100 Universities", "501-1500 Universities")

# University Rankings: 1-100 vs. 1501+
ranking_subset <- repl_df %>% 
  filter(ranking_cat %in% c("1-100", "1501+")) %>%
  mutate(is_top100_subset = ifelse(ranking_cat == "1-100", 1, 0))
detailed_comparison(ranking_subset, "pro_techno_prop", "is_top100_subset", "Top 100 Universities", "1501+ Universities")
...
ranking_subset <- repl_df %>% 
  filter(ranking_cat %in% c("1-100", "101-500")) %>%
  mutate(is_top100_subset = ifelse(ranking_cat == "1-100", 1, 0))
detailed_comparison(ranking_subset, "pro_beh_prop", "is_top100_subset", "Top 100 Universities", "101-500 Universities")

# University Rankings: 1-100 vs. 501-1500
ranking_subset <- repl_df %>% 
  filter(ranking_cat %in% c("1-100", "501-1500")) %>%
  mutate(is_top100_subset = ifelse(ranking_cat == "1-100", 1, 0))
detailed_comparison(ranking_subset, "pro_beh_prop", "is_top100_subset", "Top 100 Universities", "501-1500 Universities")

# University Rankings: 1-100 vs. 1501+
ranking_subset <- repl_df %>% 
  filter(ranking_cat %in% c("1-100", "1501+")) %>%
  mutate(is_top100_subset = ifelse(ranking_cat == "1-100", 1, 0))
detailed_comparison(ranking_subset, "pro_beh_prop", "is_top100_subset", "Top 100 Universities", "1501+ Universities")
...
################################################################################
## Comparing Two Groups for Stance Types: Expertise and Reach
################################################################################

# Update grouping variable for cultural liberalism using all four categories
repl_df <- repl_df %>%
  mutate(exp_cult = case_when(
    type_H_reach_H_exp_cult == 1 ~ "High-Reach Expert",
    type_H_reach_L_exp_cult == 1 ~ "High-Reach Non-Expert",
    type_L_reach_H_exp_cult == 1 ~ "Low-Reach Expert",
    type_L_reach_L_exp_cult == 1 ~ "Low-Reach Non-Expert",
    TRUE ~ NA_character_
  ))

# Update grouping variable for economic collectivism using all four categories
repl_df <- repl_df %>%
  mutate(exp_econ = case_when(
    type_H_reach_H_exp_econ == 1 ~ "High-Reach Expert",
    type_H_reach_L_exp_econ == 1 ~ "High-Reach Non-Expert",
    type_L_reach_H_exp_econ == 1 ~ "Low-Reach Expert",
    type_L_reach_L_exp_econ == 1 ~ "Low-Reach Non-Expert",
    TRUE ~ NA_character_
  ))

#--------------------------------------------------------------------------
# Comparison for Cultural Liberalism:
# 1. High-Reach Non-Expert vs. High-Reach Expert
#--------------------------------------------------------------------------
res_cult_highreach <- compare_two_groups(
  df = repl_df %>% filter(exp_cult %in% c("High-Reach Expert", "High-Reach Non-Expert")),
  outcome = "stance_cultural_liberalism",
  groupvar = "exp_cult",
  label_group1 = "High-Reach Expert",
  label_group2 = "High-Reach Non-Expert"
)

print(res_cult_highreach)

#--------------------------------------------------------------------------
# 2. Low-Reach Expert vs. High-Reach Non-Expert
#--------------------------------------------------------------------------
res_cult_low_vs_highnon <- compare_two_groups(
  df = repl_df %>% filter(exp_cult %in% c("Low-Reach Expert", "High-Reach Non-Expert")),
  outcome = "stance_cultural_liberalism",
  groupvar = "exp_cult",
  label_group1 = "Low-Reach Expert",
  label_group2 = "High-Reach Non-Expert"
)
print(res_cult_low_vs_highnon)

#--------------------------------------------------------------------------
# Comparison for Economic Collectivism:
# 1. High-Reach Non-Expert vs. High-Reach Expert
#--------------------------------------------------------------------------
res_econ_highreach <- compare_two_groups(
  df = repl_df %>% filter(exp_econ %in% c("High-Reach Expert", "High-Reach Non-Expert")),
  outcome = "stance_econ_collectivism",
  groupvar = "exp_econ",
  label_group1 = "High-Reach Expert",
  label_group2 = "High-Reach Non-Expert"
)
print(res_econ_highreach)

#--------------------------------------------------------------------------
# 2. Low-Reach Expert vs. High-Reach Non-Expert
#--------------------------------------------------------------------------
res_econ_low_vs_highnon <- compare_two_groups(
  df = repl_df %>% filter(exp_econ %in% c("Low-Reach Expert", "High-Reach Non-Expert")),
  outcome = "stance_econ_collectivism",
  groupvar = "exp_econ",
  label_group1 = "Low-Reach Expert",
  label_group2 = "High-Reach Non-Expert"
)
print(res_econ_low_vs_highnon)
...
################################################################################
## Gender Differences for Egocentrism
################################################################################

res_gender_ego <- compare_two_groups(
  df = repl_df,
  outcome = "egocentrism",
  groupvar = "is_male",
  label_group1 = 0,  # Female
...
################################################################################
## Gender Differences for Toxicity
################################################################################

res_gender_tox <- compare_two_groups(
  df = repl_df,
  outcome = "toxicity",
  groupvar = "is_male",
  label_group1 = 0,  # Female
...
################################################################################
## University Rankings for Emotionality-Reasoning
################################################################################

res_top100_vs_101_500_emotionality <- compare_two_groups(
  df = repl_df %>% filter(ranking_cat %in% c("1-100", "101-500")),
  outcome = "emotionality",
  groupvar = "ranking_cat",
  label_group1 = "1-100",
...
print(res_top100_vs_101_500_emotionality)

res_top100_vs_501_1500_emotionality <- compare_two_groups(
  df = repl_df %>% filter(ranking_cat %in% c("1-100", "501-1500")),
  outcome = "emotionality",
  groupvar = "ranking_cat",
  label_group1 = "1-100",
...
print(res_top100_vs_501_1500_emotionality)
