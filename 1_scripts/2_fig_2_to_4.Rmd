...
## Extended Data Figure 2: US Academics
FOR each variable in 'clim_var'
...
repl_df %<>%
  mutate(
    field_group = case_when(
      is_stem == 1 & is_soc_sci == 0 & is_humanities == 0 ~ "STEM",
      is_stem == 0 & is_soc_sci == 1 & is_humanities == 0 ~ "Soc. Sciences",
      is_stem == 0 & is_soc_sci == 0 & is_humanities == 1 ~ "Humanities",
      TRUE ~ NA_character_
    )
  ) %>%
  filter(!is.na(field_group))
```
...
stats_pol_field <- map_df(clim_var, ~calculate_stats(repl_df %>% filter(field_group == "STEM"), .x)) %>%
  mutate(Type = "STEM") %>%
  bind_rows(
    map_df(clim_var, ~calculate_stats(repl_df %>% filter(field_group == "Soc. Sciences"), .x)) %>%
      mutate(Type = "Soc. Sciences"),
    map_df(clim_var, ~calculate_stats(repl_df %>% filter(field_group == "Humanities"), .x)) %>%
      mutate(Type = "Humanities")
  )
...
p_all <- ggplot(stats_pol_comb %>% filter(Term %in% clim_var), aes(x = Mean, y = Type, color = Type, group = Type)) +
  ...
  facet_grid(rows = vars(Group), cols = vars(Label), scales = "free_y", space = "free_y", switch = "y")
...
#   mutate(stats_pol_is_stem, Type = "STEM")
# )
stats_pol_field <- map_df(econ_var, ~calculate_stats(repl_df %>% filter(field_group == "STEM"), .x)) %>%
  mutate(Type = "STEM") %>%
  bind_rows(
    map_df(econ_var, ~calculate_stats(repl_df %>% filter(field_group == "Soc. Sciences"), .x)) %>%
      mutate(Type = "Soc. Sciences"),
    map_df(econ_var, ~calculate_stats(repl_df %>% filter(field_group == "Humanities"), .x)) %>%
      mutate(Type = "Humanities")
  )
stats_pol_field$Type <- factor(stats_pol_field$Type, levels = c("Humanities", "Soc. Sciences", "STEM"))
...
p_all_fig3 <- ggplot(stats_pol_comb %>% filter(Term %in% econ_var), aes(x = Mean, y = Type, color = Type, group = Type)) +
  ...
  scale_color_manual(values = color_values) +
  scale_x_continuous(limits = c(0, 0.166)) +  # Fixes the x-axis range
...
#   mutate(stats_pol_is_stem, Type = "STEM")
# )
stats_pol_field <- map_df(beh_var, ~calculate_stats(repl_df %>% filter(field_group == "STEM"), .x)) %>%
  mutate(Type = "STEM") %>%
  bind_rows(
    map_df(beh_var, ~calculate_stats(repl_df %>% filter(field_group == "Soc. Sciences"), .x)) %>%
      mutate(Type = "Soc. Sciences"),
    map_df(beh_var, ~calculate_stats(repl_df %>% filter(field_group == "Humanities"), .x)) %>%
      mutate(Type = "Humanities")
  )
stats_pol_field$Type <- factor(stats_pol_field$Type, levels = c("Humanities", "Soc. Sciences", "STEM"))
...
# Calculate the stats for each variable for U.S. academics only
stats_main <- map_df(variables, ~calculate_stats(repl_df_us, .x))
# Reverse the order of variables on the y-axis by using rev()
stats_main$Label <- factor(stats_main$Term, levels = rev(variables),
                           labels = rev(topic_label_mapping[variables]))

# Create a single bar graph: y-axis for the 8 variables, x-axis for the Mean value
p_bar <- ggplot(stats_main, aes(x = Mean, y = Label)) +
  geom_col(fill = "#7FB3D5") +  # Lighter blue fill for improved error bar visibility
  geom_errorbarh(aes(xmin = CI_lower, xmax = CI_upper), height = 0.5, 
                 color = "black", size = 0.6) +  # Increased error bar height and line size
  labs(x = "Mean Value", y = "") +
  theme_classic(base_size = 12) +
  theme(
    axis.line = element_line(),   # Ensure x-axis line is visible
    axis.ticks = element_line(),
    axis.text = element_text(color = "black")
  )
p_bar

# Save the figure as PDF and EPS
ggsave("edf2.pdf", p_bar, width = 8, height = 6)
ggsave("edf2.eps", p_bar, width = 8, height = 6, device = cairo_ps)
```
...
Calculate the mean, standard deviation (SD), and 95% confidence intervals (CIs)
```{r}
...
compare_two_groups <- function(data, outcome, groupvar, group1, group2) {
  ...
  # maintain order
  subdata[[groupvar]] <- factor(subdata[[groupvar]],
...
variables_to_compare <- c("stance_climate_action", "pro_techno_prop", "pro_beh_prop")
...
### University Rankings;  US-based vs Non-US-based academics
```{r}
...
Calculate the mean, standard deviation (SD), and 95% confidence intervals (CIs) for 
```{r}
...
res_top100_vs_1500_cult <- compare_two_groups(
  df = repl_df %>% filter(ranking_cat %in% c("1-100","1501+")),
  outcome = "stance_cultural_liberalism",
  groupvar = "ranking_cat",
  label_group1 = "1-100",
  label_group2 = "1501+"
)
print(res_top100_vs_1500_cult)


res_top100_vs_1500_econ <- compare_two_groups(
  df = repl_df %>% filter(ranking_cat %in% c("1-100","1501+")),
  outcome = "stance_econ_collectivism",
  groupvar = "ranking_cat",
  label_group1 = "1-100",
...
# Create a new variable indicating group membership for toxicity comparison
repl_df %<>%
  mutate(ReachCred_egocentrism = case_when(
    type_H_reach_L_cred == 1 ~ "HighReachLowCred",
    type_L_reach_H_cred == 1 ~ "LowReachHighCred"
  ))

res_egocentrism_comparison <- compare_two_groups(
  df = repl_df %>% filter(ReachCred_Emot %in% c("HighReachLowCred", "LowReachHighCred")),
  outcome = "egocentrism",
  groupvar = "ReachCred_egocentrism",
  label_group1 = "HighReachLowCred",  # Group 1
  label_group2 = "LowReachHighCred"    # Group 2
)
print(res_egocentrism_comparison)
...
# Create a new variable indicating group membership for toxicity comparison
repl_df %<>%
  mutate(ReachCred_Tox = case_when(
    type_H_reach_L_cred == 1 ~ "HighReachLowCred",
    type_L_reach_H_cred == 1 ~ "LowReachHighCred"
  ))

res_toxicity_comparison <- compare_two_groups(
  df = repl_df %>% filter(ReachCred_Tox %in% c("HighReachLowCred", "LowReachHighCred")),
  outcome = "toxicity",
  groupvar = "ReachCred_Tox",
  label_group1 = "HighReachLowCred",  # Group 1
  label_group2 = "LowReachHighCred"    # Group 2
)
print(res_toxicity_comparison)
...
# Create a new variable indicating group membership for toxicity comparison
repl_df %<>%
  mutate(ReachCred_Emot = case_when(
    type_H_reach_L_cred == 1 ~ "HighReachLowCred",
    type_L_reach_H_cred == 1 ~ "LowReachHighCred"
  ))

res_emot_comparison <- compare_two_groups(
  df = repl_df %>% filter(ReachCred_Emot %in% c("HighReachLowCred", "LowReachHighCred")),
  outcome = "emotionality",
  groupvar = "ReachCred_Emot",
  label_group1 = "HighReachLowCred",  # Group 1
  label_group2 = "LowReachHighCred"    # Group 2
)
print(res_emot_comparison)
